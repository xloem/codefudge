...
options.sort(key = lambda option: sum((loss_change_per_epoch for loss, loss_change_per_epoch, authored_date in option[2]))/len(option[2]))
for rounded_lr, grad_accum, elems in options:#[-16:]:
    print(rounded_lr, grad_accum, f'{len(elems)}x',
            sum((loss_change_per_epoch for loss, loss_change_per_epoch, authored_date in elems))/len(elems),
            datetime.datetime.fromtimestamp(sum((authored_date for loss, loss_change_per_epoch, authored_date in elems))/len(elems)).isoformat()
    )
